...
public class TimelineWindow : EditorWindow
{
    ...
    private void DirectorUpdate()
    {
        ...
        if (cutscene != null)
        {
            if (!Application.isPlaying && cutscene.State == TimelineManager.TimeLineState.PreviewPlaying)
            {
    ...
    protected void OnEnable()
    {
        ...
        if (instanceId >= 0)
        {
            TimelineManager[] cutscenes = GameObject.FindObjectsOfType<TimelineManager>();
    ...
    {
        TimelineManager c = e.cutscene as TimelineManager;
        if (c != null)
        {
    ...
    private void updateToolbar(Rect toolbarArea)
    {
        ...
        if (GUILayout.Button(CREATE, EditorStyles.toolbarDropDown, GUILayout.Width(60)))
        {
            ...
            createMenu.AddItem(empty_Skill, false, createEmptyCutscene);
        ...
        // Cutscene selector
        cachedCutscenes = GameObject.FindObjectsOfType<TimelineManager>();
        if (cachedCutscenes != null && cachedCutscenes.Length > 0)
        {
            ...
            Array.Sort(cachedCutscenes, delegate (TimelineManager c1, TimelineManager c2)
            {
        ...
        if (cutscene != null)
        {
            ...
            if (GUILayout.Button("导出技能", EditorStyles.toolbarButton, GUILayout.Width(50)))
            {
                SkillExportHelper.ExportTimelineSkill(cutscene);
            }
    ...
    public void FocusCutscene(TimelineManager cutscene)
    {
    ...
    private string txtCutsceneName = "Skill";
    ...
    public TimelineManager CreateCutscene()
    {
        string cutsceneName = DirectorHelper.getCutsceneItemName(txtCutsceneName, typeof(TimelineManager));

        GameObject cutsceneGO = new GameObject(cutsceneName);
        TimelineManager cutscene = cutsceneGO.AddComponent<TimelineManager>();
       // Skill skill = cutsceneGO.AddComponent<Skill>();
